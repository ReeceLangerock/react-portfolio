...
export class Project extends React.Component {
  ...
  renderBuiltWith() {
    return projectData[this.props.projectName]["built-with"].map((item, index) => <li key={item + index}>{item}</li>);
  }
  renderDescWithLink(key) {
    if (projectData[key].descriptionLink) {
      return (
        <p>
          {projectData[key].description}{" "}
          <a href={projectData[key].descriptionLink} rel="noopener noreferrer" target="_blank">
            this
          </a>{" "}
          {projectData[key].descriptionEnd}
        </p>
      );
    } else {
      return <p>{projectData[key].description}</p>;
    }
  }
          ...
          {key !== "NONE" && (
            <div className="project__info">
              <div className="project__info-header">
                <span>{projectData[key].platform}</span>
                <span>{projectData[key]["date-completed"]}</span>
              </div>
              <h1>{this.props.projectName}</h1>
              {this.renderDescWithLink(key)}
              <h3>Built With:</h3>
              <ul>{this.renderBuiltWith()}</ul>
              <h3>
